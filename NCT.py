...
source_char_file = './en-char-list.txt'
target_char_file = './hi-char-list.txt'
training_file = './train_file.txt'
...
def predict(test_file, max_word_len=max_seq_len):
    ...
    for line in f:
        ...
        if len(line) > 0 and len(line.split('\t')) == 2:
            ...
            for i in range(len(target_str)):
                temp_y.append(target_chars.index(target_str[i]))
            # x-padding
            r = max_word_len - len(source_str)
            for i in range(r):
                temp_x.append(0)
    ...
    for i in range(len(y)):
        ...
        strs_x = strs_x.strip()
        strs_gold_y = ''
        for j in range(len(gold_yw)):
            if target_chars[gold_yw[j]] != '<P>':
                strs_gold_y += target_chars[gold_yw[j]]
        strs_gold_y = strs_gold_y.strip()
        strs_y = ''
        for j in range(len(yw)):
            ...
            if c != '<P>':
                strs_y +=c
        strs_y = strs_y.strip()
        f.write(strs_x+'\t'+strs_gold_y+'\t'+strs_y+'\n')
        f.flush()
    f.close()
